...
{
    public class EstateManagerService : IEstateManagerService
    {
        ...
        public async Task<EquineEstate?> ValidateAndGetEstateById (Guid equineEstateId)
        {
            await _estateValidationService.LoadAndValidateEstateAsync(equineEstateId);

            return await _sharedEstateCrudService.GetEstateByIdAsync(equineEstateId);
        }
